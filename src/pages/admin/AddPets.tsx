...
export const AddPet = () => {
  ...
  const onSubmit: SubmitHandler<PetFormValues> = async (data) => {
    try {
      ...
      setApiError('Ocurrió un error al guardar la mascota. Verifica los datos o el tamaño de la imagen.');
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-sm border border-gray-100">
      <div className="mb-6 border-b pb-4">
        <h1 className="text-2xl font-bold text-gray-800">Agregar Nueva Mascota</h1>
        <p className="text-gray-600">Ingresa los detalles y sube una foto.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input {...register('name')} className="w-full px-4 py-2 border rounded-md" placeholder="Ej. Bobby" />
          ...
          <div>
            ...
            <input 
              {...register('species')} 
              className={`w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.species ? 'border-red-500' : 'border-gray-300'}`} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">ID del Refugio</label>
            <input type="number" {...register('shelter', { valueAsNumber: true })} className="w-full px-4 py-2 border rounded-md" />
            {errors.shelter && <p className="mt-1 text-xs text-red-500">{errors.shelter.message}</p>}
          </div>
        </div>
        <div>
          ...
          />
          <p className="text-xs text-gray-500 mt-1">Formatos permitidos: JPG, PNG, WebP.</p>
        </div>

        <div className="flex justify-end gap-4 pt-4 border-t">
          <button type="button" onClick={() => navigate('/admin/mascotas')} className="px-6 py-2 bg-gray-100 rounded-md">Cancelar</button>
          <button type="submit" disabled={isSubmitting} className="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-blue-300">
